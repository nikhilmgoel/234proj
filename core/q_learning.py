...
class QN(object):
    ...
    def __init__(self, env, config, logger=None):
        ...
        self.episodes = range(config.num_episodes)
        if config.randomize_dataset is True: random.shuffle(self.episodes)
        # store the episodes for training
        self.train_episodes = self.episodes[0:config.num_episodes_train]

        # store the episodes for testing
        self.test_episodes = self.episodes[config.num_episodes_train:]
