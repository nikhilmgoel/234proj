...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        width = state_shape[1]
        num_channels = state_shape[2]
        batch_size = None
 
        self.s          = tf.placeholder(tf.uint8, shape    = (batch_size, height, width, state_history * num_channels))
        self.a          = tf.placeholder(tf.int32, shape    = batch_size)
        self.r          = tf.placeholder(tf.float32, shape  = batch_size)
        self.sp         = tf.placeholder(tf.uint8, shape    = (batch_size, height, width, state_history * num_channels))
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_vars = {'target_' + q.name: q for q in Q}
        assign_ops = [t.assign(q_vars[t.name]) for t in target]
    ...
    def add_loss_op(self, q, target_q):
        ...
        future_rewards = self.config.gamma * tf.reduce_max(target_q, axis = 1)
        one_if_not_done = (1.0 - tf.cast(self.done_mask, tf.float32))
        Q_sample = self.r + one_if_not_done * future_rewards
        actions_taken = tf.cast(tf.one_hot(self.a, num_actions), tf.float32)
        Q_sa = tf.reduce_sum(q * actions_taken, axis = 1)
        self.loss = tf.reduce_mean((Q_sample - Q_sa)**2)
    ...
    def add_optimizer_op(self, scope):
        ...
        grads = adam_optimizer.compute_gradients(
            self.loss, 
            tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = scope))
 
        true_grads = [(tf.clip_by_norm(g, self.config.clip_val), v) for g, v in grads] if self.config.grad_clip else grads
 
        self.train_op = adam_optimizer.apply_gradients(true_grads)
        t_list = [g for g, v in true_grads]
        self.grad_norm = tf.global_norm(t_list)
