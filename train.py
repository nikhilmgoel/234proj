...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    ...
    for n in range(0, n_layers):
      if (n == 0):
        prev_layer = layers.fully_connected(
          mlp_input,
          size)
      else:
        curr_layer = layers.fully_connected(
          prev_layer, 
          size)
    ...
    for path in paths:
      ...
      for i in range(0, len(rewards)):
        for j in range(i, len(rewards)):
          if (j > 0):
            path_returns[i] += (config.gamma ** (j-i)) * rewards[j]
          else:
            path_returns[i] += rewards[j]
  ...
  def calculate_advantage(self, returns, observations):
    ...
    if self.config.use_baseline:
      baseline = self.sess.run(self.baseline,
        feed_dict={self.baseline_target_placeholder : returns, 
